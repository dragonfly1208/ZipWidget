...
#include <QSet>
#include <QSharedData>
#define FILE_ATTRIBUTE_DIRECTORY          (0040000<<16)//0x00000010
/// \cond internal
    ...
    QDir::Filters filter, QDir::SortFlags sort, TFileInfoList &result) const
{
    ...
    bool ok;
    QuaZipFileInfo64 dotdotinfo = QuaZipDir_getFileInfo(zip, &ok, "..",false);
    dotdotinfo.externalAttr |= FILE_ATTRIBUTE_DIRECTORY;
    list.push_back(dotdotinfo);
    do {
        ...
        if (relativeName.contains('/')) {
            ...
            if (dirsFound.contains(relativeName)){
                if(isReal){
                    dirsFound.remove(relativeName);
                    for(int i=0;i<list.length();i++){
                        const QuaZipFileInfo64 & it = list.at(i);
                        if(it.name != relativeName)
                            continue;
                        list.removeAt(i);
                        break;
                    }
        ...
        if(isDir) info.externalAttr |= FILE_ATTRIBUTE_DIRECTORY;
        list.append(info);
